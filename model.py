...
def main(): 
	...
	# output.append("Positive tweets percentage: {} %".format(100*len(ptweets)/len(tweets))) 
	# picking negative tweets from tweets 
	pos=(100*len(ptweets)/len(tweets))
	
	ntweets = [tweet for tweet in tweets if tweet['sentiment'] == 'negative'] 
	# percentage of negative tweets 
	# output.append("Negative tweets percentage: {} %".format(100*len(ntweets)/len(tweets))) 
	# percentage of neutral tweets 
	# output.append("Neutral tweets percentage: {} % ".format(100*(len(tweets) - len(ntweets) - len(ptweets))/len(tweets))) 
	neg=(100*len(ntweets)/len(tweets))
	neu=(100*(len(tweets) - len(ntweets) - len(ptweets))/len(tweets))
	data={"pos":pos,"neg":neg,"neu":neu}
	import json
